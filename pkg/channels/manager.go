...
func (m *Manager) SetupHTTPServer(addr string, healthServer *health.Server) {
	...
	m.registerChannelHandlers(m.mux)
...
// RegisterOnMux registers channel handlers (webhooks, health checks) on the given mux.
// This allows channels to be served by a shared HTTP server alongside other components.
func (m *Manager) RegisterOnMux(mux *http.ServeMux) {
	m.registerChannelHandlers(mux)
}

// GetMux returns the HTTP mux for registering additional handlers.
// Returns nil if SetupHTTPServer hasn't been called.
...
// registerChannelHandlers registers webhook and health handlers on the given mux.
func (m *Manager) registerChannelHandlers(mux *http.ServeMux) {
