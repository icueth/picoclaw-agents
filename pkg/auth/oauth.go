...
func GoogleAntigravityOAuthConfig() OAuthProviderConfig {
	...
	if clientID == "" {
		// Fallback to internal base64 encoded placeholder or instruction
		clientID = decodeBase64("W0VOVEVSX1lPVVJfQ0xJRU5UX0lEX0hFUkVd")
	}
	clientSecret := os.Getenv("ANTIGRAVITY_CLIENT_SECRET")
	if clientSecret == "" {
		clientSecret = decodeBase64("W0VOVEVSX1lPVVJfQ0xJRU5UX1NFQ1JFVF9IRVJFXQ==")
	}
	return OAuthProviderConfig{
...
// MiniMaxPortalRegion represents the MiniMax API region (cn or global).
type MiniMaxPortalRegion string

const (
	MiniMaxPortalRegionGlobal MiniMaxPortalRegion = "global"
	MiniMaxPortalRegionCN     MiniMaxPortalRegion = "cn"
)

// MiniMaxPortalOAuthConfig returns the OAuth configuration for MiniMax Portal.
// MiniMax uses a custom device-code flow (not standard RFC 8628).
func MiniMaxPortalOAuthConfig(region MiniMaxPortalRegion) OAuthProviderConfig {
	const clientID = "78257093-7e40-4613-99e0-527b14b39113"
	var baseURL string
	if region == MiniMaxPortalRegionCN {
