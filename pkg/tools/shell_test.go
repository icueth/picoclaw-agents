...
// TestShellTool_URLPathsNotBlocked verifies that URL path components (e.g.
// wttr.in/Bangkok) are not treated as filesystem paths outside the workspace.
func TestShellTool_URLPathsNotBlocked(t *testing.T) {
	tmpDir := t.TempDir()
	tool, err := NewExecTool(tmpDir, true)
	if err != nil {
		t.Fatalf("unable to configure exec tool: %s", err)
	...
	commands := []string{
		`curl -s "https://wttr.in/Bangkok?format=3"`,
		`curl -s https://wttr.in/Bangkok?format=%l:+%c+%t`,
		`wget -q https://example.com/path/to/file.txt`,
	...
	for _, cmd := range commands {
		result := tool.Execute(context.Background(), map[string]any{"command": cmd})
		if result.IsError && strings.Contains(result.ForLLM, "path outside working dir") {
			t.Errorf("URL path should not be blocked by workspace check: %s\n  error: %s", cmd, result.ForLLM)
		}
	}
}

// TestShellTool_SafetyLevel_Permissive verifies that permissive mode only blocks
// truly destructive operations (rm -rf, disk wipe, fork bomb, curl|bash).
func TestShellTool_SafetyLevel_Permissive(t *testing.T) {
	cfg := &config.Config{
	...
	for _, cmd := range allowed {
		result := tool.Execute(context.Background(), map[string]any{"command": cmd})
		if result.IsError && strings.Contains(result.ForLLM, "blocked") {
			t.Errorf("permissive: should allow %q, got blocked: %s", cmd, result.ForLLM)
		}
	}

	// These SHOULD still be blocked in permissive mode
	blocked := []string{
		"rm -rf /",
	...
	for _, cmd := range blocked {
		result := tool.Execute(context.Background(), map[string]any{"command": cmd})
		if !result.IsError {
			t.Errorf("permissive: should block %q", cmd)
		}
	}
}

// TestShellTool_SafetyLevel_Balanced verifies balanced mode blocks privilege
// escalation but allows common dev operations.
func TestShellTool_SafetyLevel_Balanced(t *testing.T) {
	cfg := &config.Config{
		Tools: config.ToolsConfig{
			Exec: config.ExecConfig{
				EnableDenyPatterns: true,
	...
	for _, cmd := range allowed {
		result := tool.Execute(context.Background(), map[string]any{"command": cmd})
		if result.IsError && strings.Contains(result.ForLLM, "blocked") {
			t.Errorf("balanced: should allow %q, got blocked: %s", cmd, result.ForLLM)
		}
	}

	// Blocked in balanced (privilege escalation)
	blocked := []string{
		"sudo su",
		"sudo -i",
		"apt purge nginx",
	...
	for _, cmd := range blocked {
		result := tool.Execute(context.Background(), map[string]any{"command": cmd})
		if !result.IsError {
			t.Errorf("balanced: should block %q", cmd)
		}
	}
}

// TestShellTool_SafetyLevel_Strict verifies strict mode blocks everything
// including common dev operations (legacy behavior).
func TestShellTool_SafetyLevel_Strict(t *testing.T) {
	cfg := &config.Config{
		Tools: config.ToolsConfig{
			Exec: config.ExecConfig{
				EnableDenyPatterns: true,
	...
	// All of these should be blocked in strict mode
	blocked := []string{
		`echo $(date)`,
		`echo ${HOME}`,
		`git push origin main`,
	...
	for _, cmd := range blocked {
		result := tool.Execute(context.Background(), map[string]any{"command": cmd})
		if !result.IsError {
			t.Errorf("strict: should block %q", cmd)
		}
	}
}
