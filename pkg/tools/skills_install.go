...
type InstallSkillTool struct {
	...
	workspace       string // Fallback: workspace-specific skills (for backward compatibility)
	mu              sync.Mutex
}
...
// globalSkillsDir is the global skills directory (default: ~/.picoclaw/skills/).
// workspace is the workspace directory (fallback for backward compatibility).
func NewInstallSkillTool(registryMgr *skills.RegistryManager, globalSkillsDir, workspace string) *InstallSkillTool {
...
func (t *InstallSkillTool) Execute(ctx context.Context, args map[string]any) *ToolResult {
	...
	// Check if already installed (prefer global skills directory).
	// Default to global directory so all agents can share skills.
	...
	output += "\nThe skill is now available globally and can be used by all agents."
