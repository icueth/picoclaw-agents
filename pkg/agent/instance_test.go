...
func TestNewAgentInstance_ResolveCandidatesFromModelListAlias(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "custom vLLM server with slash in model name",
			aliasName:    "qwen3.5-custom-server",
			modelName:    "vllm/Qwen/Qwen3.5-35B-A3B-FP8",
			apiBase:      "http://123.253.61.68:8000/v1",
			wantProvider: "vllm",
			wantModel:    "Qwen/Qwen3.5-35B-A3B-FP8",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Verify agent.Model is the resolved API model name, not the alias
			if agent.Model != tt.wantModel {
				t.Fatalf("agent.Model = %q, want %q (resolved from alias %q)", agent.Model, tt.wantModel, tt.aliasName)
			}
		})
	}
}
