...
func NewMessageBus() *MessageBus {
	return &MessageBus{
		...
		events:        make(chan AgentEvent, defaultBusBufferSize),
...
func (mb *MessageBus) PublishEvent(ctx context.Context, event AgentEvent) error {
	if mb.closed.Load() {
		return ErrBusClosed
	}
	if err := ctx.Err(); err != nil {
...
func (mb *MessageBus) SubscribeEvent(ctx context.Context) (AgentEvent, bool) {
	select {
	case event, ok := <-mb.events:
		return event, ok
	case <-mb.done:
