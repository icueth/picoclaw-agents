...
func ConvertProvidersToModelList(cfg *Config) []ModelConfig {
	...
	migrations := []providerMigrationConfig{
		...
		{
			providerNames: []string{"minimax-portal", "minimax"},
			protocol:      "minimax-portal",
			buildConfig: func(p ProvidersConfig) (ModelConfig, bool) {
				if p.MiniMaxPortal.APIKey == "" && p.MiniMaxPortal.APIBase == "" && p.MiniMaxPortal.AuthMethod == "" {
					return ModelConfig{}, false
				}
				return ModelConfig{
					ModelName:      "minimax-m2.5",
					Model:          "minimax-portal/MiniMax-M2.5",
					APIKey:         p.MiniMaxPortal.APIKey,
					APIBase:        p.MiniMaxPortal.APIBase,
					Proxy:          p.MiniMaxPortal.Proxy,
					RequestTimeout: p.MiniMaxPortal.RequestTimeout,
					AuthMethod:     p.MiniMaxPortal.AuthMethod,
				}, true
			},
		},
		{
			providerNames: []string{"kimi-coding", "kimi"},
			protocol:      "kimi-coding",
			buildConfig: func(p ProvidersConfig) (ModelConfig, bool) {
				if p.KimiCoding.APIKey == "" && p.KimiCoding.APIBase == "" {
					return ModelConfig{}, false
				}
				return ModelConfig{
					ModelName:      "kimi-for-coding",
					Model:          "kimi-coding/kimi-for-coding",
					APIKey:         p.KimiCoding.APIKey,
					APIBase:        p.KimiCoding.APIBase,
					Proxy:          p.KimiCoding.Proxy,
					RequestTimeout: p.KimiCoding.RequestTimeout,
				}, true
			},
		},
	}
