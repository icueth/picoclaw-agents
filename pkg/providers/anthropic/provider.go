...
func (p *Provider) Chat(
	...
	onChunk, hasOnChunk := options["on_chunk"].(func(string))

	// Always use streaming to satisfy MiniMax Portal's "streaming is required" error
	// when max_tokens is high, and to handle long-running operations (>10 min).
	stream := p.client.Messages.NewStreaming(ctx, params, opts...)
	var accumulatedResponse anthropic.Message

	for stream.Next() {
		event := stream.Current()
		if err := accumulatedResponse.Accumulate(event); err != nil {
			return nil, fmt.Errorf("anthropic stream accumulation failed: %w", err)
		}

		// Handle on_chunk callback if provided and not nil
		if hasOnChunk && onChunk != nil {
			if deltaEv, ok := event.AsAny().(anthropic.ContentBlockDeltaEvent); ok {
				if textDelta, ok := deltaEv.Delta.AsAny().(anthropic.TextDelta); ok {
					if textDelta.Text != "" {
	...
	return parseResponse(&accumulatedResponse), nil
}
...
func buildParams(
	...
	for i < len(messages) {
		...
		case "user":
			if msg.ToolCallID != "" {
				// We found a starting tool result. Group it with all subsequent tool results.
				var blocks []anthropic.ContentBlockParamUnion
				j := i
				for j < len(messages) {
					m := messages[j]
					if (m.Role == "user" || m.Role == "tool") && m.ToolCallID != "" {
		...
		case "assistant":
			if len(msg.ToolCalls) > 0 {
				...
				for _, tc := range msg.ToolCalls {
					...
					// DEBUG: Print the payload being injected
					fmt.Printf("DEBUG: Injecting input for %s: %#v\n", name, args)
			...
			i++
		case "tool":
			// Group "tool" role messages just like "user" with ToolCallID.
			var blocks []anthropic.ContentBlockParamUnion
			j := i
			for j < len(messages) {
				m := messages[j]
