...
func CreateProvider(cfg *config.Config) (LLMProvider, string, error) {
	...
	if cfg.HasProvidersConfig() {
		...
		for _, m := range cfg.ModelList {
			...
			existingModels[m.Model] = true
		}
		for _, pm := range providerModels {
			// Skip if already exists by ModelName OR by Model (provider/model-id)
			if !existingModelNames[pm.ModelName] && !existingModels[pm.Model] {
