...
// TestCreateProviderFromConfig_CustomServer verifies that an unknown protocol
// with api_base set is treated as a custom OpenAI-compatible server,
// and the FULL model name (including slashes) is passed through.
func TestCreateProviderFromConfig_CustomServer(t *testing.T) {
	cfg := &config.ModelConfig{
		ModelName: "qwen3.5-custom-server",
		Model:     "Qwen/Qwen3.5-35B-A3B-FP8",
		APIBase:   "http://123.253.61.68:8000/v1",
		APIKey:    "test-key",
	}
	...
	// Full model name must be preserved (not split on /)
	if modelID != "Qwen/Qwen3.5-35B-A3B-FP8" {
		t.Errorf("modelID = %q, want %q", modelID, "Qwen/Qwen3.5-35B-A3B-FP8")
	}
}

// TestCreateProviderFromConfig_CustomServerNoSlash verifies custom server
// with a simple model name (no slash) also works.
func TestCreateProviderFromConfig_CustomServerNoSlash(t *testing.T) {
	cfg := &config.ModelConfig{
	...
	// No slash = defaults to openai protocol, modelID = "my-custom-model"
	if modelID != "my-custom-model" {
		t.Errorf("modelID = %q, want %q", modelID, "my-custom-model")
	}
}
