...
func TestResolveProviderSelection(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			setup: func(cfg *config.Config) {
				cfg.ModelList = nil // Clear to test legacy ProvidersConfig
		...
		},
		{
			...
			setup: func(cfg *config.Config) {
				...
				// Clear ModelList to force fallback to ProvidersConfig
				// (ModelList has priority and would override ProvidersConfig)
				cfg.ModelList = nil
			},
