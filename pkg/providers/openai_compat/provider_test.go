...
func TestNormalizeModel_PreservesHuggingFaceOrgNames(t *testing.T) {
	// HuggingFace org names have uppercase — they are part of the model name,
	// not provider prefixes. normalizeModel must NOT strip them.
	tests := []struct {
		model string
		want  string
	}{
		{"Qwen/Qwen3.5-35B-A3B-FP8", "Qwen/Qwen3.5-35B-A3B-FP8"},
		{"NVIDIA/Llama-3.1-Nemotron-70B", "NVIDIA/Llama-3.1-Nemotron-70B"},
		{"Meta/llama-3.3-70b", "Meta/llama-3.3-70b"},
		// Lowercase provider prefixes should still be stripped
		{"bailian/qwen3.5-plus", "qwen3.5-plus"},
		{"vllm/my-model", "my-model"},
		{"ollama/llama3", "llama3"},
		// No slash — return as-is
		{"gpt-4o", "gpt-4o"},
	}
	for _, tt := range tests {
		if got := normalizeModel(tt.model, "http://localhost:8000/v1"); got != tt.want {
