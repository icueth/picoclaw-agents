...
const supportedProvidersMsg = "supported providers: openai, anthropic, google-antigravity, minimax-portal, minimax-portal-cn"
...
func authLoginCmd(provider string, useDeviceCode bool) error {
	...
	case "minimax-portal", "minimax":
		return authLoginMiniMaxPortal(auth.MiniMaxPortalRegionGlobal)
	case "minimax-portal-cn", "minimax-cn":
		return authLoginMiniMaxPortal(auth.MiniMaxPortalRegionCN)
	default:
...
func authLoginOpenAI(useDeviceCode bool) error {
	...
	if err == nil {
		...
		appCfg.Agents.Defaults.Model = "openai/gpt-5.2"
	...
	fmt.Println("Default model set to: openai/gpt-5.2")
...
func authLoginGoogleAntigravity() error {
	...
	if err == nil {
		...
		appCfg.Agents.Defaults.Model = "antigravity/gemini-3-flash"
...
func authLoginMiniMaxPortal(region auth.MiniMaxPortalRegion) error {
	cfg := auth.MiniMaxPortalOAuthConfig(region)

	cred, err := providers.LoginMiniMaxPortal(cfg)
	if err != nil {
		return fmt.Errorf("login failed: %w", err)
	}

	// Store region so token refresh knows which endpoint to use
	regionStr := "global"
	if region == auth.MiniMaxPortalRegionCN {
		regionStr = "cn"
	}
	cred.ProjectID = regionStr
	...
	appCfg, err := internal.LoadConfig()
	if err == nil {
		// Update or add minimax-portal in ModelList
		found := false
		for i := range appCfg.ModelList {
			if isMiniMaxPortalModel(appCfg.ModelList[i].Model) {
				appCfg.ModelList[i].AuthMethod = "oauth"
				found = true
				break
			}
		...
		// Update default model
		appCfg.Agents.Defaults.Model = "minimax-portal/MiniMax-M2.5"
	...
	fmt.Printf("\n✓ MiniMax Portal (%s) login successful!\n", regionLabel)
	fmt.Println("Default model set to: minimax-portal/MiniMax-M2.5")
	fmt.Println("Try it: picoclaw agent -m \"Hello world\"")
...
func authLoginPasteToken(provider string) error {
	...
	if err == nil {
		...
		case "anthropic":
			...
			// Update default model
			appCfg.Agents.Defaults.Model = "anthropic/claude-sonnet-4.6"
		case "openai":
	...
	if provider != "" {
		...
		if err == nil {
			...
			for i := range appCfg.ModelList {
				...
				case "minimax-portal", "minimax", "minimax-portal-cn", "minimax-cn":
					if isMiniMaxPortalModel(appCfg.ModelList[i].Model) {
						appCfg.ModelList[i].AuthMethod = ""
					}
				}
...
// isMiniMaxPortalModel checks if a model string belongs to minimax-portal provider
func isMiniMaxPortalModel(model string) bool {
	return model == "minimax-portal" ||
		strings.HasPrefix(model, "minimax-portal/")
}
